...
export default function VerifyEmailScreen() {
  ...
  const handleVerify = async () => {
    ...
    try {
      const res = await apiRequestDirect("POST", "http://13.127.188.130:3001/auth/verify-email", { otp: otpValue, email });
  ...
  const handleResend = async () => {
    ...
    try {
      const res = await apiRequestDirect("POST", "http://13.127.188.130:3001/auth/send-email-otp", { email });
      const rawText = await res.text();
      let data: any = {};
      try { data = JSON.parse(rawText); } catch { data = { message: rawText }; }
      if (res.ok) {
    ...
    <KeyboardAvoidingView style={[styles.root, { paddingTop: topPad }]} behavior={Platform.OS === "ios" ? "padding" : undefined}>
      <ScrollView contentContainerStyle={[styles.container, { paddingBottom: insets.bottom + 32 }]} keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.stepsRow}>
          ...
          <StepItem n="2" text="Mobile OTP" state="done" />
          <View style={styles.stepLine} />
        ...
        <View style={styles.card}>
          <View style={styles.emailBadge}>
            ...
            <Text style={styles.emailBadgeText} numberOfLines={1}>OTP sent to {email}</Text>
          </View>
...
const styles = StyleSheet.create({
  ...
  emailBadge: { flexDirection: "row", alignItems: "center", gap: 7, backgroundColor: Colors.primaryLight, borderRadius: 10, paddingHorizontal: 12, paddingVertical: 8, borderWidth: 1, borderColor: Colors.primary + "33", maxWidth: "100%" },
  emailBadgeText: { fontSize: 13, fontFamily: "Urbanist_500Medium", color: Colors.primary, flexShrink: 1 },

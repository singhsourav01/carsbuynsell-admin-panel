...
function SellSheet({ visible, onClose }: { visible: boolean; onClose: () => void }) {
  ...
  const handleSubmit = async () => {
    ...
    setLoading(true);
    try {
      const res = await apiRequestDirect("POST", "http://localhost:8002/user/sell-request", { title: title.trim(), description: description.trim(), category, listingType, basePrice: parseInt(basePrice, 10) }, true);
...
export default function ProfileScreen() {
  ...
  const fetchProfile = useCallback(async () => {
    try {
      setProfileError("");
      const res = await apiRequestDirect("GET", "http://localhost:8002/user/users-profile", undefined, true);
      const rawText = await res.text();
      let data: any = {};
  ...
  const handleSaveProfile = async () => {
    ...
    setSaving(true);
    try {
      const res = await apiRequestDirect("PUT", "http://localhost:8002/user/users-profile", {
